...
export class OnboardingFlow {
  ...
  // ─────────────────────────────────────────────────────────────────────────
  // Slot application (called from postback handler or message handler for freetext)
  // ─────────────────────────────────────────────────────────────────────────

  async applySlot(zaloUserId: string, step: OnboardingStep, value: string): Promise<void> {
    const context = await this.state.getContext(zaloUserId);
    ...
    switch (step) {
      ...
      case 'mode': {
        ...
        if (teachingMode === 'online') {
          // Online tutoring — area is not relevant, skip to freetext
          await this.state.updateContext(zaloUserId, { onboardingStep: 'freetext' });
          await this.askFreetext(zaloUserId);
        } else {
      ...
      case 'area': {
        if (value === 'other') {
          // User chose "Tỉnh khác" — wait for free-text city name
          // Step stays 'area' so message handler knows to accept free text here
          await this.zalo.sendText(
            zaloUserId,
            'Bạn đang ở tỉnh/thành phố nào? Bạn nhắn tên tỉnh/thành phố nhé (ví dụ: Cần Thơ, Bình Dương).',
          );
        } else {
